...
# date: 2023-0706
# description: RECCE detector (inference-only port)
...
Note: self.backbone is kept solely to match the pretrained checkpoint state dict.
Actual inference runs through self.model (Recce with timm Xception encoder).
"""
...
from timm.models import xception as timm_xception
...
encoder_params = {
    "xception": {
        "features": 2048,
        # pretrained=False: we load weights from the full checkpoint
...
class GraphReasoning(nn.Module):
    ...
    def forward(self, vert_a, vert_b, vert_c):
        ...
        for j in range(emb_vert_a.shape[-1]):
            ...
            agg_vb.append(
                torch.bmm(emb_v_b.transpose(1, 2), w).squeeze() * gate_vert_b[:, :, j]
            )
            ...
            agg_vc.append(
                torch.bmm(emb_v_c.transpose(1, 2), w).squeeze() * gate_vert_c[:, :, j]
            )
        ...
        agg_vert_abc = agg_vert_abc.reshape(
            vert_a.shape[0], -1, vert_a.shape[2], vert_a.shape[3]
        )
...
class Recce(nn.Module):
    ...
    def features(self, x):
        ...
        recons_x = F.interpolate(
            pred, size=x.shape[-2:], mode="bilinear", align_corners=True
        )
